const { InteractionType, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, MessageFlags } = require('discord.js');
...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (['lfg-join', 'lfg-leave', 'lfg-close'].includes(customId)) {
            await handleLFGInteraction(interaction, client);
            return;
        }
...
async function handleLFGInteraction(interaction, client) {
    const { customId, user, message } = interaction;

    try {
        const lfgSession = await LFG.findOne({ messageId: message.id });

        if (!lfgSession) {
            return interaction.reply({ content: 'This LFG post has expired or is no longer valid.', flags: [MessageFlags.Ephemeral] });
        }
...
async function handleRPSInteraction(interaction, client) {
    const { customId, user, message } = interaction;

    try {
        if (!client.rpsChallenges) {
            return interaction.reply({ content: 'No active RPS challenges found.', flags: [MessageFlags.Ephemeral] });
        }

        const challengeData = client.rpsChallenges.get(message.id);
        if (!challengeData) {
            return interaction.reply({ content: 'This challenge has expired or is no longer valid.', flags: [MessageFlags.Ephemeral] });
        }

        switch (customId) {
            case 'rps_accept':
                await handleRPSAccept(interaction, challengeData, client);
                break;
            case 'rps_decline':
                await handleRPSDecline(interaction, challengeData, client);
                break;
            case 'rps_rock':
            case 'rps_paper':
            case 'rps_scissors':
                await handleRPSMove(interaction, challengeData, client);
                break;
        }
    } catch (error) {
...
async function handleRPSAccept(interaction, challengeData, client) {
    if (interaction.user.id !== challengeData.opponent) {
        return interaction.reply({ content: 'This challenge is not for you.', flags: [MessageFlags.Ephemeral] });
    }

    if (challengeData.status !== 'pending') {
        return interaction.reply({ content: 'This challenge has already been responded to.', flags: [MessageFlags.Ephemeral] });
    }
    ...
    const gameEmbed = new EmbedBuilder()
        ...
        .addFields(
            { name: 'Player 1', value: `<@${challengeData.challenger}>`, inline: true },
    ...
    await interaction.reply({ content: 'Challenge accepted! The game has begun!', flags: [MessageFlags.Ephemeral] });
    ...
    setTimeout(() => {
        if (client.rpsChallenges.has(interaction.message.id)) {
            ...
            if (data.status === 'active' && (!data.moves[data.challenger] || !data.moves[data.opponent])) {
                const timeoutEmbed = new EmbedBuilder()
                    .setColor(0xFF0000)
                    .setTitle('⏰ Game Timeout')
                    .setDescription('One or both players did not make their moves in time!')
                    .setTimestamp();
...
async function handleRPSDecline(interaction, challengeData, client) {
    ...
    await interaction.reply({ content: 'You have declined the challenge.', flags: [MessageFlags.Ephemeral] });
    client.rpsChallenges.delete(interaction.message.id);
}
...
async function handleRPSMove(interaction, challengeData, client) {
    ...
    if (playerId !== challengeData.challenger && playerId !== challengeData.opponent) {
    ...
    if (challengeData.status !== 'active') {
        return interaction.reply({ content: 'This game is not active.', flags: [MessageFlags.Ephemeral] });
    }

    if (challengeData.moves[playerId]) {
        return interaction.reply({ content: 'You have already made your move!', flags: [MessageFlags.Ephemeral] });
    }

    // Record the move
    challengeData.moves[playerId] = move;

    await interaction.reply({ content: `You chose ${getMoveEmoji(move)}!`, flags: [MessageFlags.Ephemeral] });

    // Check if both players have moved
    if (challengeData.moves[challengeData.challenger] && challengeData.moves[challengeData.opponent]) {
...
async function determineWinner(interaction, challengeData, client) {
    ...
    const resultEmbed = new EmbedBuilder()
        ...
        .addFields(
            { name: `<@${challengeData.challenger}>`, value: getMoveEmoji(challengerMove), inline: true },
            { name: 'VS', value: '⚔️', inline: true },
            { name: `<@${challengeData.opponent}>`, value: getMoveEmoji(opponentMove), inline: true },
    ...
    if (winner !== 'tie') {
        const winnerId = winner === 'challenger' ? challengeData.challenger : challengeData.opponent;
        const loserId = winner === 'challenger' ? challengeData.opponent : challengeData.challenger;

        try {
            const [winnerUser, loserUser] = await Promise.all([
                User.findOrCreate(winnerId, interaction.guildId, {
                    username: interaction.client.users.cache.get(winnerId)?.username || 'Unknown',
                    discriminator: interaction.client.users.cache.get(winnerId)?.discriminator || '0',
                    avatar: interaction.client.users.cache.get(winnerId)?.avatar
                }),
                User.findOrCreate(loserId, interaction.guildId, {
                    username: interaction.client.users.cache.get(loserId)?.username || 'Unknown',
                    discriminator: interaction.client.users.cache.get(loserId)?.discriminator || '0',
        ...
        try {
            const [challengerUser, opponentUser] = await Promise.all([
                User.findOrCreate(challengeData.challenger, interaction.guildId, {
                    username: interaction.client.users.cache.get(challengeData.challenger)?.username || 'Unknown',
                    discriminator: interaction.client.users.cache.get(challengeData.challenger)?.discriminator || '0',
                    avatar: interaction.client.users.cache.get(challengeData.challenger)?.avatar
                }),
                User.findOrCreate(challengeData.opponent, interaction.guildId, {
                    username: interaction.client.users.cache.get(challengeData.opponent)?.username || 'Unknown',
                    discriminator: interaction.client.users.cache.get(challengeData.opponent)?.discriminator || '0',
                    avatar: interaction.client.users.cache.get(challengeData.opponent)?.avatar
                })
            ]);
...
async function handleJoin(interaction, lfgSession) {
    if (lfgSession.status !== 'open') {
        return interaction.reply({ content: 'This LFG is no longer open for joining.', flags: [MessageFlags.Ephemeral] });
    }

    if (lfgSession.players.includes(interaction.user.id)) {
        return interaction.reply({ content: "You've already joined this group!", flags: [MessageFlags.Ephemeral] });
    }

    if (lfgSession.creatorId === interaction.user.id && !lfgSession.players.includes(interaction.user.id)) {
    ...
    await interaction.reply({ content: 'You have successfully joined the group!', flags: [MessageFlags.Ephemeral] });
}

async function handleLeave(interaction, lfgSession) {
    if (!lfgSession.players.includes(interaction.user.id)) {
        return interaction.reply({ content: "You aren't in this group.", flags: [MessageFlags.Ephemeral] });
    }

    lfgSession.players = lfgSession.players.filter(id => id !== interaction.user.id);
    ...
    await lfgSession.save();
    await updateLFGMessage(interaction, lfgSession);
    await interaction.reply({ content: 'You have left the group.', flags: [MessageFlags.Ephemeral] });
}

async function handleClose(interaction, lfgSession) {
    if (interaction.user.id !== lfgSession.creatorId) {
        return interaction.reply({ content: 'Only the creator of this LFG can close it.', flags: [MessageFlags.Ephemeral] });
    }
...
async function updateLFGMessage(interaction, lfgSession) {
    ...
    const playersList = lfgSession.players.map(id => `<@${id}>`).join('\n') || 'None yet!';
    newEmbed.setFields(
        { name: 'Slots Available', value: `**${lfgSession.slots - lfgSession.players.length}**`, inline: true },

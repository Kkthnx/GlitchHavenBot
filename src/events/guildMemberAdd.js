  ...
  async execute(member) {
    try {
      ...
      if (guild.welcome.enabled) {
        await handleWelcome(member, guild);
      }
      ...
      logger.info(
        `New member joined: ${member.user.tag} in ${member.guild.name}`,
      );
    } catch (error) {
...
async function handleWelcome(member, guild) {
  try {
...
async function handleAutoRoles(member, guild) {
  try {
    ...
    if (rolesToAdd.length > 0) {
      ...
      logger.info(
        `Assigned auto-roles to ${member.user.tag}: ${rolesToAdd.map((r) => r.name).join(", ")}`,
      );
    }
  } catch (error) {

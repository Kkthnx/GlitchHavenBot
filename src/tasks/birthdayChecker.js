...
const checkBirthdays = async (client) => {
  try {
    ...
    logger.info(
      `[Birthday] Found ${birthdayUsers.length} user(s) with a birthday today.`,
    );
    ...
    for (const guildId in birthdaysByGuild) {
      ...
      if (
        guildSettings &&
        guildSettings.birthdays.enabled &&
        guildSettings.birthdays.channelId
      ) {
        const channel = await client.channels
          .fetch(guildSettings.birthdays.channelId)
          .catch(() => null);
        if (channel) {
          ...
          for (const user of usersInGuild) {
            const member = await channel.guild.members
              ...
              .catch(() => null);
            if (member) {
              const message = guildSettings.birthdays.message.replace(
                "{user}",
                member.toString(),
              );
...
const scheduleBirthdayCheck = (client) => {
  ...
  logger.info(
    "[Scheduler] Birthday check task scheduled to run daily at 9:00 AM UTC.",
  );
};

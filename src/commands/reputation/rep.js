    ...
    try {
      if (!args || args.length === 0) {
        return interaction.reply(
          "❌ Please specify a user to give reputation to. Usage: `!rep @user [message]`",
        );
      }
      ...
      try {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `**${giver.username}** has endorsed **${targetUser.username}**!`,
          )
          .addFields(
            {
              name: "New Reputation Score",
              value: `${targetData.reputation.score}`,
              inline: true,
            },
            {
              name: "Total Endorsements",
              value: `${targetData.reputation.totalEndorsements}`,
              inline: true,
            },
          ...
          .setTimestamp()
          .setFooter({
            text: `Reputation System • ${giverData.reputation.repGivenToday}/3 used today`,
          });
        ...
        if (error.message === "You have already endorsed this user") {
          const alreadyEmbed = new EmbedBuilder()
            ...
            .setDescription(
              `You have already given reputation to **${targetUser.username}**.\nYou can only endorse each user once!`,
            )
            .setFooter({ text: "Reputation System" });
      ...
      const errorEmbed = new EmbedBuilder()
        ...
        .setTitle("❌ Error")
        .setDescription(
          "An error occurred while processing your reputation request.",
        )
        .setFooter({ text: "Reputation System" });

...
async function handleAdopt(message, options) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${message.author.username}** has adopted a new **${petType}**!`,
      )
      .addFields(
      ...
      .setFooter({
        text: "Use !pet feed, !pet play, and !pet train to care for your pet!",
      })
      .setTimestamp();
...
async function handleFeed(message) {
  try {
    ...
    if (!pet) {
      return message.reply(
        "❌ You don't have a pet! Use `!pet adopt` to get one.",
      );
    }
    ...
    if (timeSinceLastFed < 60 * 60 * 1000) {
      const minutesLeft = Math.ceil(
        (60 * 60 * 1000 - timeSinceLastFed) / (60 * 1000),
      );
      return message.reply(
        `⏰ ${pet.name} isn't hungry yet! Try again in ${minutesLeft} minutes.`,
      );
    }
...
async function handlePlay(message) {
  try {
    ...
    if (timeSinceLastPlayed < 15 * 60 * 1000) {
      const minutesLeft = Math.ceil(
        (15 * 60 * 1000 - timeSinceLastPlayed) / (60 * 1000),
      );
      return message.reply(
        `⏰ ${pet.name} is tired from playing! Try again in ${minutesLeft} minutes.`,
      );
    }
...
async function handleTrain(message) {
  try {
    ...
    if (timeSinceLastTrained < 60 * 60 * 1000) {
      const minutesLeft = Math.ceil(
        (60 * 60 * 1000 - timeSinceLastTrained) / (60 * 1000),
      );
      return message.reply(
        `⏰ ${pet.name} needs rest from training! Try again in ${minutesLeft} minutes.`,
      );
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: "⚔️ Attack",
          value: `+${statGain} (${pet.stats.attack})`,
          inline: true,
        },
        { name: "⚡ Energy", value: `${pet.energy}%`, inline: true },
        { name: "⭐ Experience", value: `+${expGain} XP`, inline: true },
...
async function handleBattle(message, args) {
  try {
    ...
    if (pet.energy < 30) {
      return message.reply(
        `❌ ${pet.name} is too tired to battle! They need at least 30% energy.`,
      );
    }
...
async function handleShop(message) {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "🦜 Parrot",
        value: "Colorful and intelligent bird",
        inline: true,
      },

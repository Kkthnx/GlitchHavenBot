  ...
  try {
    if (options.length < 2) {
      return message.reply(
        "❌ Please provide a game name and max players. Usage: `!turn create <game_name> <max_players>`",
      );
    }
    ...
    if (existingGame) {
      return message.reply(
        "❌ You already have an active game! Use `!turn end` to end it first.",
      );
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${message.author.username}** created a new game: **${gameName}**`,
      )
      .addFields(
      ...
      .setFooter({
        text: "Use !turn join <game_id> to join, !turn start to begin!",
      })
      .setTimestamp();
...
async function handleJoin(message, args) {
  try {
    ...
    if (!gameId) {
      return message.reply(
        "❌ Please provide a game ID. Use `!turn list` to see available games.",
      );
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: "👥 Players",
          value: `${game.players.length}/${game.maxPlayers}`,
          inline: true,
        },
        {
          name: "📊 Status",
...
async function handleStart(message, args) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: "Use !turn next to advance turns, !turn skip to skip a player",
      })
      .setTimestamp();
    ...
    try {
      await currentMember.send(
        `🎮 It's your turn in **${game.name}**! Check the game channel.`,
      );
    } catch (dmError) {
...
async function handleNext(message, args) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({
        text: "Use !turn next to advance, !turn skip to skip a player",
      })
      .setTimestamp();
...
async function handleSkip(message, args) {
  try {
    ...
    const newCurrentPlayer = game.players.find(
      (p) => p.userId === newCurrentPlayerId,
    );
    const newCurrentMember =
      await message.guild.members.fetch(newCurrentPlayerId);

    const embed = new EmbedBuilder()
      .setColor(0xe74c3c)
      .setTitle("⏭️ Player Skipped!")
      .setDescription(
        `**${currentPlayer.username}** was skipped in **${game.name}**`,
      )
      .addFields(
        {
          name: "🎯 New Current Player",
          value: `**${newCurrentPlayer.username}**`,
          inline: true,
        },
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "📊 Total Turns",
          value: (game.turnCount || 0).toString(),
          inline: true,
        },
        {
          name: "⏱️ Duration",
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: "📊 Status",
          value: game.status === "active" ? "Active" : "Waiting for players",
          inline: true,
        },
        {
          name: "👥 Players",
    ...
    if (game.status === "active") {
      const currentPlayer = game.players.find(
        (p) => p.userId === game.turnOrder[game.currentTurn],
      );
      embed.addFields(
        {
          name: "🎯 Current Turn",
...
async function handleList(message) {
  try {
    ...
    if (games.length === 0) {
      return message.reply(
        "❌ No active games found. Use `!turn create` to start one!",
      );
    }
...
async function showHelp(message) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "Manage turn-based games and notify players of their turns!",
    )
    .addFields(
      {
        name: "🎮 Commands",
    ...
    .setFooter({
      text: "Perfect for D&D, board games, and other turn-based activities!",
    })
    .setTimestamp();

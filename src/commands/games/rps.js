  ...
  async execute(message, args, client) {
    ...
    if (args.length === 0) {
      return message.reply(
        "❌ Please mention a user to challenge! Usage: `!rps @user`",
      );
    }
    ...
    const challengeEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `${message.author} has challenged ${targetUser} to a game of Rock, Paper, Scissors!`,
      )
      .addFields(
      ...
      if (client.rpsChallenges.has(challengeMsg.id)) {
        ...
        if (data.status === "pending") {
          const expiredEmbed = EmbedBuilder.from(challengeEmbed)
            .setColor(0xff0000)
            .setDescription(
              `${message.author}'s challenge to ${targetUser} has expired!`,
            )
            .spliceFields(2, 1, {
              name: "Status",

  ...
  try {
    ...
    if (existingAdventure) {
      return message.reply(
        "❌ There's already an active adventure! Use `!adventure status` to see the current story.",
      );
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${message.author.username}** has started a new **${theme}** adventure!`,
      )
      .addFields(
        { name: "📖 Current Story", value: adventure.story, inline: false },
        {
          name: "🎯 Your Choices",
          value: formatChoices(adventure.choices),
          inline: false,
        },
      ...
      .setFooter({
        text: "Use !adventure join to participate, !adventure vote <choice> to vote!",
      })
      .setTimestamp();
...
async function handleJoin(message) {
  try {
    ...
    if (!adventure) {
      return message.reply(
        "❌ No active adventure found. Use `!adventure start` to begin one!",
      );
    }

    if (adventure.participants.includes(message.author.id)) {
      return message.reply(
        "❌ You're already participating in this adventure!",
      );
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${message.author.username}** has joined the adventure!`,
      )
      .addFields(
        {
          name: "👥 Total Participants",
          value: adventure.participants.length.toString(),
          inline: true,
        },
        { name: "📖 Current Story", value: adventure.story, inline: false },
      )
      .setFooter({
        text: "Use !adventure vote <choice> to help decide the story!",
      })
      .setTimestamp();
...
async function handleVote(message, choiceArgs) {
  try {
    ...
    if (!adventure.participants.includes(message.author.id)) {
      return message.reply(
        "❌ You need to join the adventure first! Use `!adventure join`",
      );
    }
    ...
    if (!choice || choice < 1 || choice > adventure.choices.length) {
      return message.reply(
        `❌ Please vote for a valid choice (1-${adventure.choices.length}).`,
      );
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${message.author.username}** voted for: **${adventure.choices[choice - 1]}**`,
      )
      .addFields(
        {
          name: "📊 Vote Progress",
          value: `${totalVotes}/${participants} votes cast`,
          inline: true,
        },
        {
          name: "⏰ Status",
          value:
            totalVotes >= Math.ceil(participants * 0.5)
              ? "Ready to progress!"
...
async function handleStatus(message) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${adventure.theme}** Adventure - Chapter ${adventure.currentChapter}`,
      )
      .addFields(
        { name: "📖 Current Story", value: adventure.story, inline: false },
        {
        ...
        {
          name: "🗳️ Votes Cast",
          value: Object.keys(adventure.votes).length.toString(),
          inline: true,
        },
        {
          name: "📊 Vote Results",
          value: formatVoteResults(voteCounts, adventure.choices),
          inline: false,
        },
      )
    ...
    return message.reply(
      "❌ An error occurred while fetching adventure status.",
    );
  }
}
...
async function showHelp(message) {
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "Embark on collaborative storytelling adventures with your server!",
    )
    .addFields(
      {
        name: "🎮 Commands",
      ...
      {
        name: "🎨 Themes",
        value: "Fantasy, Sci-Fi, Mystery, Horror, Comedy, and more!",
      },
    )
...
async function progressStory(adventure, channel) {
  try {
    ...
    const nextStory = generateNextStory(
      adventure.theme,
      winningChoice,
    ...
    if (adventure.currentChapter > 5) {
      ...
      const endEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `The **${adventure.theme}** adventure has reached its conclusion!`,
        )
        .addFields(
          ...
          {
            name: "📝 Total Chapters",
            value: adventure.currentChapter.toString(),
            inline: true,
          },
        )
        .setFooter({
          text: "Thanks for playing! Start a new adventure anytime!",
        })
        .setTimestamp();
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `The community chose: **${adventure.choices[winningChoice - 1]}**`,
      )
      .addFields(
...
function generateInitialStory(theme) {
  const stories = {
    fantasy:
      "You find yourself at the entrance of an ancient forest. The trees whisper secrets of forgotten magic, and a mysterious light flickers in the distance. Three paths lie before you...",
    scifi:
      "Your spaceship has crash-landed on an unknown planet. The air is breathable, but strange sounds echo from the alien landscape. Your survival depends on your next choice...",
    mystery:
      "A detective's office in the dead of night. A mysterious client has left an envelope with three cryptic clues. Each leads to a different aspect of the case...",
    horror:
      "You wake up in an abandoned mansion. The air is thick with dread, and you hear distant footsteps. Three doors stand before you, each hiding different terrors...",
    comedy:
      "You're a clumsy wizard who just graduated from magic school. Your first day on the job, and you've already turned your boss into a frog. Now you need to fix this...",
    western:
      "The dusty town of Deadwood Gulch stretches before you. A wanted poster catches your eye, and three different leads promise to bring justice to the lawless frontier...",
  };
...
function generateNextStory(theme, choice, chapter) {
  ...
  return (
    templates[choice - 1] ||
    "The story continues with new challenges and discoveries..."
  );
}
...
function formatVoteResults(voteCounts, choices) {
  ...
  return Object.entries(voteCounts)
    .map(
      ([choice, count]) => `${choice}. ${choices[choice - 1]}: ${count} votes`,
    )
    .join("\n");
}

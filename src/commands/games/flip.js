...
module.exports = {
  ...
  async execute(message) {
    try {
      ...
      if (
        user.gameStats.lastFlip &&
        now - user.gameStats.lastFlip.getTime() < cooldownTime
      ) {
        const timeLeft = Math.ceil(
          (cooldownTime - (now - user.gameStats.lastFlip.getTime())) / 1000,
        );
        return message.reply(
          `⏰ Please wait ${timeLeft} seconds before flipping again.`,
        );
      }
      ...
      const flipEmbed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "📊 Your Stats",
            value: getStatsText(user.gameStats.coinFlips),
            inline: true,
          },
          {
            name: "🔥 Current Streak",
            value: `${user.gameStats.coinFlips.streak}`,
            inline: true,
          },
      ...
      if (
        guild.games.coinFlip.streakRewards &&
        user.gameStats.coinFlips.streak > 0
      ) {
      ...
      return message.reply(
        "❌ An error occurred while processing the coin flip.",
      );
    }
  },
};

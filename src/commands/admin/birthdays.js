  ...
  async execute(message, args) {
    ...
    if (!subcommand) {
      return message.reply(
        "Please specify a subcommand: `enable`, `disable`, `channel`, or `message`.",
      );
    }
    ...
    switch (subcommand.toLowerCase()) {
      ...
      case "channel": {
        ...
        await guildData.save();
        message.reply(
          `✅ Birthday announcements will now be sent to ${channel}.`,
        );
        break;
      }
      case "message": {
        ...
        if (!newMessage) {
          return message.reply(
            "Please provide a new birthday message. Use `{user}` to mention the user.",
          );
        }
      ...
      default:
        message.reply(
          "Invalid subcommand. Use `enable`, `disable`, `channel`, or `message`.",
        );
    }
  },
};

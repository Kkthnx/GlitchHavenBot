...
module.exports = {
    ...
    async addSystemHealth(embed, client) {
        ...
        embed.addFields({
            ...
            value: `**Uptime:** ${uptimeHours}h ${uptimeMinutes}m ${uptimeSeconds}s\n` +
                `**Ping:** ${ping}ms\n` +
                `**Guilds:** ${guildCount}\n` +
                `**Users:** ${userCount}\n` +
    ...
    async addDatabaseHealth(embed) {
        ...
        try {
            ...
            const collectionCounts = Object.values(stats.collections || {}).reduce((a, b) => a + b, 0);

            embed.addFields({
                name: '🗄️ Database Status',
                value: `${dbColor} **Status:** ${dbStatus}\n` +
    ...
    async addCacheHealth(embed) {
        ...
        embed.addFields({
            name: '💾 Cache Status',
            value: `${cacheColor} **Hit Rate:** ${overallHitRate}%\n` +
    ...
    async addPerformanceHealth(embed) {
        ...
        embed.addFields({
            ...
            value: `${queryHealth} **Queries:** ${perfStats.queries.total} (${perfStats.queries.slow} slow)\n` +
    ...
    async addMemoryHealth(embed) {
        ...
        embed.addFields({
            name: '🧠 Memory Status',
            value: `${memoryColor} **Usage:** ${memStats.usageRatio}%\n` +

...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!subcommand) {
            return message.reply('Please specify a subcommand: `add`, `remove`, or `list`.');
        }
...
async function handleAdd(message, levelArg, roleArg) {
    ...
    if (role.position >= message.guild.members.me.roles.highest.position) {
        return message.reply("I cannot manage this role. Please make sure my role is higher than the reward role.");
    }

    try {
        const guildData = await Guild.findOneAndUpdate(
            { guildId: message.guild.id },
            { $pull: { 'leveling.roleRewards': { level: level } } },
            { new: true }
        );
        ...
        message.reply(`✅ Successfully set **${role.name}** as the reward for reaching **Level ${level}**.`);
    } catch (error) {
...
async function handleRemove(message, levelArg) {
    const level = parseInt(levelArg);
    if (isNaN(level) || level <= 0) {
        return message.reply('Please provide a valid level number to remove the reward from.');
    }
    ...
    try {
        ...
        if (result.modifiedCount > 0) {
            message.reply(`✅ Successfully removed the role reward for **Level ${level}**.`);
        } else {
            message.reply(`⚠️ No role reward was found for **Level ${level}**.`);
        }
    } catch (error) {
        logger.error("Error removing level reward:", error);
        return message.reply('❌ An error occurred while removing the level reward.');
    }
}
...
async function handleList(message) {
    try {
        ...
        if (!guildData || !guildData.leveling.roleRewards || guildData.leveling.roleRewards.length === 0) {
            return message.reply('There are no level rewards configured for this server.');
        }

        const sortedRewards = guildData.leveling.roleRewards.sort((a, b) => a.level - b.level);

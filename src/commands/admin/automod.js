...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        const autoMod = guildData.moderation.autoMod;
        ...
        switch (subcommand ? subcommand.toLowerCase() : 'status') {
            ...
            default:
                return message.reply('Invalid subcommand. Use `status`, `toggle <feature>`, `addword <word>`, `removeword <word>`, or `listwords`.');
        }
    }
};
...
function showStatus(message, autoMod) {
    const statusEmbed = new EmbedBuilder()
        ...
        .addFields(
            { name: 'System Enabled', value: autoMod.enabled ? '✅ Yes' : '❌ No', inline: true },
            { name: 'Word Filter', value: autoMod.wordFilter ? '✅ Enabled' : '❌ Disabled', inline: true },
            { name: 'Link Filter', value: autoMod.linkFilter ? '✅ Enabled' : '❌ Disabled', inline: true },
            { name: 'Spam Detection', value: `Threshold: ${autoMod.spamThreshold} msgs / 10s`, inline: true },
            { name: 'Banned Words Count', value: `\`${autoMod.bannedWords.length}\` words`, inline: true }
        )
        .setTimestamp();
...
async function handleToggle(message, feature, autoMod, guildData) {
    if (!feature) return message.reply("Please specify a feature to toggle: `system`, `wordfilter`, `linkfilter`.");
    ...
    switch (feature.toLowerCase()) {
        ...
        case 'linkfilter':
            ...
            break;
        default:
            return message.reply("Invalid feature. Use `system`, `wordfilter`, or `linkfilter`.");
    }

    await guildData.save();
    return message.reply(`${enabled ? '✅' : '❌'} ${featureName} has been **${enabled ? 'enabled' : 'disabled'}**.`);
}

...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (args.length > 0) {
      ...
      const commandEmbed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: "Usage",
            value: `\`${client.config.prefix}${command.usage || command.name}\``,
            inline: true,
          },
          {
            name: "Cooldown",
          ...
          {
            name: "Category",
            value: getCommandCategory(command.name),
            inline: true,
          },
        )
    ...
    const helpEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `Use \`${client.config.prefix}help <command>\` for detailed information about a specific command.`,
      )
      .setThumbnail(client.user.displayAvatarURL())
      .setFooter({
        text: `Total Commands: ${commands.size} | Prefix: ${client.config.prefix}`,
      })
      .setTimestamp();

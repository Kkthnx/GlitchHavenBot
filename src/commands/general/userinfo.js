...
module.exports = {
  ...
  async execute(message, args) {
    try {
      ...
      const infoEmbed = new EmbedBuilder()
        ...
        .addFields(
...

          {
            ...
            value: `${formatAbsoluteTime(user.createdAt)}\n(${formatRelativeTime(user.createdAt)})`,
            inline: true,
          },
          {
            name: "Joined Server",
            value: `${formatAbsoluteTime(member.joinedAt)}\n(${formatRelativeTime(member.joinedAt)})`,
            inline: true,
          },
          {
            name: "Booster",
            value: member.premiumSince
              ? `Since ${formatRelativeTime(member.premiumSince)}`
              : "No",
            inline: true,
          },
      ...
      if (dbUser) {
        const warnings = dbUser.moderation.warnings.filter(
          (w) => w.active,
        ).length;
        ...
        infoEmbed.addFields(
          ...
          {
            name: "Last Seen",
            value: formatRelativeTime(dbUser.lastSeen),
            inline: true,
          },
          {
            name: "Reputation",
            value: `**Score:** ${dbUser.reputation.score}\n**Rank:** #${repRank}\n**Endorsements:** ${dbUser.reputation.totalEndorsements}`,
            inline: true,
          },
        );
      ...
      if (dbUser && dbUser.lastSeen) {
        const timeSinceLastSeen =
          Date.now() - new Date(dbUser.lastSeen).getTime();

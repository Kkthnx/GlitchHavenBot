    ...
    async execute(message, args, client) {
        ...
        switch (subcommand) {
            ...
            case 'list':
                ...
                break;
            case 'view':
                await handleView(message, client);
                break;
            case 'clear':
...
async function handleSet(message, args) {
    ...
    if (!backgroundName) {
        return message.reply('Please provide the name of the background you want to set. Use `!rankcard list` to see available backgrounds.');
    }

    const backgroundsPath = path.join(__dirname, '../../../assets/rankcards');
    const availableBackgrounds = fs.readdirSync(backgroundsPath)
        .filter(file => file.endsWith('.png'))
        .map(file => file.replace('.png', ''));

    if (!availableBackgrounds.includes(backgroundName)) {
        return message.reply('That background does not exist. Use `!rankcard list` to see available backgrounds.');
    }
    ...
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`Your rank card background has been set to **${backgroundName}**.`)
...
async function handleView(message, client) {
    try {
...
async function handleClear(message) {
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription('Your rank card background has been reset to the default.')
...
async function showHelp(message) {
    const embed = new EmbedBuilder()
        ...
        .addFields(
            { name: '`!rankcard set <name>`', value: 'Set your rank card background.' },
            { name: '`!rankcard list`', value: 'List available backgrounds.' },
            { name: '`!rankcard view`', value: 'View your current rank card.' },
            { name: '`!rankcard clear`', value: 'Reset your rank card to the default.' }
        );
    await message.reply({ embeds: [embed] });
} 
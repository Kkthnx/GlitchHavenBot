  ...
  async execute(message, args) {
    if (!args[0]) {
      return message.reply(
        "Please provide your birthday in `MM/DD` format. For example: `!setbirthday 07/21`.",
      );
    }
    ...
    if (!match) {
      return message.reply(
        "Invalid date format. Please use `MM/DD` (e.g., `07/21`).",
      );
    }
    ...
    if (date.getMonth() + 1 !== month || date.getDate() !== day) {
      return message.reply(
        "That's not a real date! Please provide a valid date.",
      );
    }
    ...
    try {
      ...
      const successEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `Your birthday has been successfully set to **${String(month).padStart(2, "0")}/${String(day).padStart(2, "0")}**.`,
        )

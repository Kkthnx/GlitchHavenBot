...
try {
    ...
    if (fs.existsSync(fontPath)) {
        const fonts = fs.readdirSync(fontPath).filter(file => file.endsWith('.ttf') || file.endsWith('.otf'));
        fonts.forEach(font => {
            registerFont(path.join(fontPath, font), { family: font.replace(/\.[^/.]+$/, '') });
        });
    }
} catch (error) {
...
class RankCardGenerator {
    ...
    async createRankCard(userData, backgroundName, clientUser) {
        if (!userData || !userData.username) {
            throw new Error('Invalid userData');
        }

        const themeColor = this.colors[Math.floor(Math.random() * this.colors.length)];
        ...
        if (backgroundName && backgroundName !== 'default' && fs.existsSync(backgroundsPath)) {
            try {
                const files = fs.readdirSync(backgroundsPath);
                const userBgFile = files.find(file => path.parse(file).name.toLowerCase() === backgroundName.toLowerCase());
                if (userBgFile) {
                    backgroundPath = path.join(backgroundsPath, userBgFile);
                    console.log(`[RankCard] Using user-defined background: ${userBgFile}`);
                }
            } catch (e) {
                console.error(`[RankCard] Error reading background directory for user BG:`, e);
            }
        }
        ...
        if (!backgroundPath && fs.existsSync(backgroundsPath)) {
            try {
                const availableBgs = fs.readdirSync(backgroundsPath)
                ...
                if (availableBgs.length > 0) {
                    ...
                    console.log(`[RankCard] Using random background: ${randomBg}`);
                }
            } catch (e) {
                console.error(`[RankCard] Error selecting random background:`, e);
            }
        }
        ...
        if (backgroundPath) {
            try {
                ...
                console.error(`[RankCard] FAILED to load background image at ${backgroundPath}. Drawing solid color.`, e);
                ctx.fillStyle = '#2C2F33';
                ctx.fillRect(0, 0, this.width, this.height);
            }
        } else {
            console.log(`[RankCard] No background image available. Drawing solid color.`);
            ctx.fillStyle = '#2C2F33';
            ctx.fillRect(0, 0, this.width, this.height);
        }
        ...
        const decorationURL = user.avatarDecorationURL({ extension: 'png', size: 256 });
        ...
        try {
            ...
            if (decoration) {
                ...
                ctx.drawImage(decoration, decorationX, decorationY, decorationSize, decorationSize);
            }
        } catch (error) {
            console.error('Failed to load avatar/decoration:', error);
            this.drawPlaceholderAvatar(ctx, avatarX, avatarY, avatarSize);
        }

        // --- Status Indicator ---
        const status = clientUser?.presence?.status || 'offline';
        this.drawStatusIndicator(ctx, avatarX + avatarSize, avatarY + avatarSize, status);
        ...
        ctx.fillStyle = 'white';
        const measuredUser = ctx.measureText(userData.username);
        ctx.fillText(userData.username, textX, userY, this.width - textX - this.padding);

        if (userData.discriminator && userData.discriminator !== '0') {
            ctx.font = '16px Arial';
            ctx.fillStyle = '#B9BBBE';
            ctx.fillText(`#${userData.discriminator}`, textX + measuredUser.width + 2, userY);
        }
        ...
        const fmt = xp => (xp > 999 ? `${(xp / 1000).toFixed(1)}K` : xp.toString());
        ctx.font = '16px Arial';
        ctx.fillStyle = '#B9BBBE';
        ctx.textAlign = 'right';
        ctx.fillText(`${fmt(userData.currentXp)} / ${fmt(userData.neededXp)} XP`, barX + barWidth, barY - 10);

        this.drawProgressBar(ctx, barX, barY, barWidth, barHeight, userData.currentXp, userData.neededXp, themeColor);
    ...
    drawProgressBar(ctx, x, y, width, height, currentXp, neededXp, themeColor) {
        ...
        if (progress > 0) {
            if (fillWidth < height) {
                ...
                ctx.beginPath();
                ctx.arc(x + cornerRadius, y + cornerRadius, cornerRadius, 0, 2 * Math.PI);
                ctx.fill();
            } else {
...
const createRankCard = async (userData, backgroundName, clientUser) => {
    const generator = new RankCardGenerator();
    return await generator.createRankCard(userData, backgroundName, clientUser);
};

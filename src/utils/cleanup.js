...
class CleanupUtility {
  ...
  async cleanupExpiredData() {
    try {
      ...
      if (expiredGames.deletedCount > 0) {
        logger.info(
          `Cleaned up ${expiredGames.deletedCount} expired game sessions`,
        );
      }
      ...
      if (expiredPets.deletedCount > 0) {
        logger.info(
          `Cleaned up ${expiredPets.deletedCount} expired pet battles`,
        );
      }
      ...
      if (expiredAdventures.deletedCount > 0) {
        logger.info(
          `Cleaned up ${expiredAdventures.deletedCount} expired adventures`,
        );
      }
    } catch (error) {
    ...
    try {
      ...
      for (const collection of collections) {
        try {
          ...
          logger.debug(
            `Compact not available for ${collection}:`,
            error.message,
          );
        }
      }
      ...
      for (const collection of collections) {
        try {
          ...
          logger.debug(
            `Could not update stats for ${collection}:`,
            error.message,
          );
        }
      }
  ...
  getCleanupStats() {
    ...
    const hoursSinceLastCleanup = Math.floor(
      timeSinceLastCleanup / (1000 * 60 * 60),
    );

    return {
      lastCleanup: this.lastCleanup,
      timeSinceLastCleanup,
      hoursSinceLastCleanup,

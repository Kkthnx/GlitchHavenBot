...
const initializeBot = async () => {
  try {
    ...
    logger.info(
      `Initial memory usage: ${memStats.heapUsed}MB / ${memStats.heapTotal}MB (${memStats.usageRatio}%)`,
    );

    // Log cleanup stats
    const cleanupStats = cleanupUtility.getCleanupStats();
    logger.info(
      `Cleanup utility initialized. Next cleanup in ${cleanupStats.hoursSinceLastCleanup} hours`,
    );
  } catch (error) {

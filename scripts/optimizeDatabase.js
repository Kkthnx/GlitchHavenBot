...
async function optimizeDatabase() {
  try {
    ...
    Object.entries(stats.cache).forEach(([name, cacheStats]) => {
      console.log(
        `  ${name}: ${cacheStats.hitRate} hit rate, ${cacheStats.size} entries`,
      );
    });
  ...
  for (const [collectionName, filter] of Object.entries(collections)) {
    try {
      ...
      if (result.deletedCount > 0) {
        console.log(
          `  Cleaned up ${result.deletedCount} old ${collectionName} documents`,
        );
      }
    } catch (error) {
...
async function analyzeQueryPerformance() {
  try {
    ...
    console.log(
      "  Could not analyze query performance (profiling may be disabled)",
    );
  }
}

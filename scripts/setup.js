...
async function setup() {
  ...
  envVars.MONGODB_URI =
    (await question("MongoDB URI (or press Enter for localhost): ")) ||
    "mongodb://localhost:27017/gridkeeper-bot";

  // Bot Configuration
  envVars.BOT_OWNER_ID = await question(
    "Bot Owner ID (your Discord user ID): ",
  );
  envVars.BOT_PREFIX =
    (await question("Command Prefix (press Enter for !): ")) || "!";

  // Optional Configuration
  const logLevel =
    (await question("Log Level (press Enter for info): ")) || "info";
  envVars.LOG_LEVEL = logLevel;

  const welcomeChannel = await question(
    "Welcome Channel ID (optional, press Enter to skip): ",
  );
  if (welcomeChannel) envVars.WELCOME_CHANNEL_ID = welcomeChannel;

  const logChannel = await question(
    "Log Channel ID (optional, press Enter to skip): ",
  );
  if (logChannel) envVars.LOG_CHANNEL_ID = logChannel;

  const modRole = await question(
    "Moderator Role ID (optional, press Enter to skip): ",
  );
  if (modRole) envVars.MODERATOR_ROLE_ID = modRole;

  const mutedRole = await question(
    "Muted Role ID (optional, press Enter to skip): ",
  );
